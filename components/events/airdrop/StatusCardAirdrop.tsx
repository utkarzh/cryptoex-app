...
import { IeoVendor_int } from "../types";
...
const StatusCardAirdrop: FC<Props> = ({ data }) => {
    ...
    <>
      {data.length > 0 ? (
        <div className="flex flex-wrap justify-center gap-6  ">
          {data.map((item, idx) => {
            const cardStatus = getStatus(item.startdays, item.enddays);
            return (
              <Link
                key={idx}
                href={`/airdrop/${item.vendors_vendorshortcode}`}
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2 mt-6">
                  <Image
                    src={
                      item.airdropcoins_logopath
                        ? item.airdropcoins_logopath
                        : item.airdropcoins_bannerimage
                    }
                ...
                <div className="text-xs font-light flex flex-col gap-3 mt-8 mb-2">
                  <p className="text-xs font-light  flex justify-between">
                    <span>{t("start")}:</span>
                    <span>{item.airdropcoins_startdate} (UTC)</span>
                  </p>
                  <p className="text-xs font-light  flex justify-between">
                    <span>{t("end")}:</span>
                    <span>{item.airdropcoins_enddate} (UTC)</span>
                  </p>
                </div>
              </Link>
            );
...
export default StatusCardAirdrop;
