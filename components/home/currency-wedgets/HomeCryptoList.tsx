...
const HomeCryptoList: FC<Props> = ({ isExpended, title, value, onSelect }) => {
  ...
  return (
    <div
      ...
      }   max-h-screen overflow-x-auto overflow-y-hidden scrollbar-custom  rounded-md px-2  flex flex-col gap-4 transition-all duration-700 ease-in-out`}
    >
      <div
      ...
      >
        {/* heading */}
        <div
          className="w-fit gap-1 flex items-center justify-center"
        ...
        >
          ...
          <h2
            className={`text-nowrap font-medium text-[18px] cursor-pointer ${saira.className}`}
          >
            {title}
          </h2>
        </div>
